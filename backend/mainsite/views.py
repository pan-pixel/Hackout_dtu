...
def clean():
    ...
    # Read the text file
    with open('/Users/prathamarora/Documents/dtu_hack/backend/output.txt', 'r', encoding='utf-8') as file:
        text = file.read()
...
def initial_setup(doc, topic):
    ...
    img_path = '/Users/prathamarora/Documents/dtu_hack/backend/static/assests/header.png' 
...
def graph1(symbol, doc):
    ...
    plt.title(f'Live Stock Price Graph ({stock_symbol})')

    buffer = BytesIO()
    plt.savefig(buffer, format='png')
    buffer.seek(0)

    paragraph = doc.add_paragraph(f"Here is a live stock price graph for {stock_symbol} over 3 years:")
    paragraph.paragraph_format.left_indent = Pt(24)
    paragraph.paragraph_format.right_indent = Pt(24)
    paragraph.paragraph_format.line_spacing = Inches(0.3)
    paragraph.space_after = Pt(8)
...
def graph2(symbol, doc):
    ...
    plt.xlabel('Date')
    plt.ylabel('Volume')
    plt.title(f'Live Volume Graph ({stock_symbol})')

    buffer = BytesIO()
    plt.savefig(buffer, format='png')
    buffer.seek(0)

    paragraph = doc.add_paragraph(f"Here is a live volume graph for {stock_symbol}:")
    paragraph.paragraph_format.left_indent = Pt(24)
    paragraph.paragraph_format.right_indent = Pt(24)
    paragraph.paragraph_format.line_spacing = Inches(0.3)
    paragraph.space_after = Pt(8)
